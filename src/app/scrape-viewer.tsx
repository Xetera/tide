          ...
          return (
            <div class='max-w-4xl mx-auto flex flex-col gap-6'>
              <div class='flex items-start justify-between gap-4'>
                ...
                >
                  <span class={`text-xs px-2 py-1 rounded-md font-mono shrink-0 ${entry().status === 'submitted' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-500'}`}>
                    {entry().httpStatus}
                  </span>
                </Show>
              </div>
              ...
              <Show when={entry().serverResponse !== undefined}>
                <details class='border border-border rounded-md overflow-hidden group/response'>
                  <summary class='list-none cursor-pointer select-none flex items-center gap-2 px-3 py-2 text-xs font-medium'>
                    <svg class='w-3 h-3 text-muted-foreground shrink-0 transition-transform group-open/response:rotate-90' viewBox='0 0 12 12' fill='currentColor'>
                      <path d='M4 2l4 4-4 4' stroke='currentColor' stroke-width='1.5' fill='none' stroke-linecap='round' stroke-linejoin='round'/>
                    </svg>
                    <span class='text-muted-foreground uppercase tracking-wider'>Server Response</span>
                  </summary>
              ...
              <details class='border border-border rounded-md overflow-hidden group/payload' open>
                <summary class='list-none cursor-pointer select-none flex items-center gap-2 px-3 py-2 text-xs font-medium'>
                  ...
                  <span class='text-muted-foreground uppercase tracking-wider'>Patches</span>
                  <span class='font-mono text-muted-foreground ml-auto'>{entry().patches.length}</span>
                </summary>
                <div class='border-t border-border'>
