      ...
      >
        ...
        <Show when={result.matched}>
          <span class='text-muted-foreground shrink-0'>
            {(result as Extract<LoaderMatchResult, { matched: true }>).patches.length} patches
          </span>
        </Show>
        <Show when={!result.matched && (result as Extract<LoaderMatchResult, { matched: false }>).error}>
                ...
                <>
                  <Show when={r.validationErrors.length > 0}>
                    <div class='px-2 py-1.5 flex flex-col gap-0.5'>
                      <span class='text-muted-foreground mb-0.5'>validation errors</span>
                      <For each={r.validationErrors}>
                        {(err) => <span class='font-mono text-destructive'>{err}</span>}
                      </For>
                    </div>
                  </Show>
...
function CaptureRow({
  ...
  return (
    <div class={`rounded-md border text-xs transition-colors ${selected ? 'border-foreground' : 'border-transparent hover:border-border'}`}>
      <div class='flex items-center gap-2 px-2 py-1.5 cursor-pointer' onClick={toggle}>
        ...
        <Show when={matches() !== null && !loading()}>
          <span class={`shrink-0 ${matchedCount() > 0 ? 'text-green-500' : 'text-muted-foreground'}`}>
            {matchedCount()} match{matchedCount() === 1 ? '' : 'es'}
          </span>
          <Show when={matches()!.some((r) => r.matched && (r as Extract<LoaderMatchResult, { matched: true }>).validationErrors.length > 0)}>
      ...
      <Show when={selected && matches() !== null}>
        <div class='border-t border-border px-2 py-2 flex flex-col gap-1.5'>
          ...
          <Show when={matches()!.every((r) => !r.matched)}>
            <p class='text-muted-foreground px-1 py-0.5'>No loaders matched this request.</p>
          </Show>
          <Show when={matches()!.some((r) => !r.matched && (r as Extract<LoaderMatchResult, { matched: false }>).error)}>
        ...
        <For each={captures()}>
          {(capture) => (
            <CaptureRow
              ...
              onSelect={() => setSelected((prev) => prev === capture.id ? null : capture.id)}
            />
          )}
        </For>
      </div>
