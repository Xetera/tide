    ...
    apply: function (target, thisArg, args) {
      ...
      promise.then(function (response) {
        ...
        response.clone().text().then(function (body) {
          try { JSON.parse(body); } catch (_) { return; }
          var resolvedUrl = new URL(url, window.location.origin).href;
          var requestHeaders = {};
          if (init && init.headers) {
            new Headers(init.headers).forEach(function (v, k) { requestHeaders[k] = v; });
          }
          var requestBody = (init && init.body != null)
            ? (typeof init.body === 'string' ? init.body.slice(0, 200000) : '[binary]')
            : null;
          enqueue({ url: resolvedUrl, method: method, body: body, requestBody: requestBody, requestHeaders: requestHeaders, responseHeaders: responseHeaders, status: response.status, capturedAt: Date.now() });
        });
      });
      return promise;
    }
  ...
  XMLHttpRequest.prototype.send = new Proxy(XMLHttpRequest.prototype.send, {
    apply: function (target, thisArg, args) {
      thisArg.addEventListener('load', function () {
        try { JSON.parse(thisArg.responseText); } catch (_) { return; }
        var headers = {};
        ...
        var requestBody = typeof args[0] === 'string' ? args[0].slice(0, 200000) : null;
        enqueue({ url: xhrUrl, method: xhrMethod, body: thisArg.responseText, requestBody: requestBody, requestHeaders: {}, responseHeaders: headers, status: thisArg.status, capturedAt: Date.now() });
      });
      return Reflect.apply(target, thisArg, args);
    }
  });
